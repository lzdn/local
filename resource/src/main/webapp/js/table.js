	...
	showUserTable: function(data) {
		...
		if(tbody) {
			...
			$.each(responseContext.body.page.results, function(idx, item) {
				var newNode = "<tr><td>" + (idx + 1) + "</td><td>" + item.userId + "</td><td>" + item.userName + "</td><td>" + item.email + "</td><td><a href=\"javascript:void();\" onclick=\"deleteUserByUserId(" + item.userId + ")\">删除</a>|<a href=\"javascript:void();\">修改</a></td></tr>";
